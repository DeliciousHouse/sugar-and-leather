...
// Keep a hard request ceiling even though accepted reporter text is much smaller.
...
function pathnameOnly(value) {
  ...
  try {
    const route = new URL(raw, 'https://feedback.invalid');
    if (route.origin !== 'https://feedback.invalid' || route.search || route.hash) return '';
    return route.pathname.slice(0, LIMITS.path);
  } catch {
    return '';
  }
}
...
function validate(obj) {
  ...
  const path = pathnameOnly(obj.path);
